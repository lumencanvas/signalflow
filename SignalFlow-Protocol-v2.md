...
### 2.2.1 Why MessagePack for Payload?

v1 specified a custom binary format. This was wrong. Reasons for MessagePack:

1. **Existing implementations everywhere**: JS, Python, C, Rust, Go, embedded (msgpack-c is 50KB)
2. **Self-describing**: Don't need schema to parse
3. **Compact**: Smaller than JSON, comparable to protobuf
4. **Debuggable**: Tools exist to inspect MessagePack
...
| Type | MessagePack | Bytes | Description |
|------|-------------|-------|-------------|
| `null` | nil | 1 | No value |
| `bool` | bool | 1 | Boolean |
| `i8`-`i64` | int | 1-9 | Signed integers |
| `u8`-`u64` | uint | 1-9 | Unsigned integers |
| `f32` | float32 | 5 | 32-bit float |
| `f64` | float64 | 9 | 64-bit float |
| `str` | str | 1+ | UTF-8 string |
| `bin` | bin | 1+ | Binary data |

## 6.2 Creative Primitives (Extension Types)

MessagePack extension types for creative data:
...
Standard MessagePack arrays and maps work directly:
