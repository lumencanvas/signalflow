...
> CLASP uses efficient binary encoding for compact, high-performance messaging.
> SET messages are just 31 bytes. Encode/decode at 8-11M msg/s.
...
CLASP uses compact binary encoding with positional fields:
...
### 2.2.2 Why Binary Encoding?

CLASP uses compact binary instead of JSON/MessagePack because:

1. **55% smaller**: SET message 31 bytes vs ~80 bytes JSON
2. **4x faster encode**: 8M msg/s vs ~2M msg/s
3. **5x faster decode**: 11M msg/s vs ~2M msg/s
...
### From WebSocket JSON
- Replace JSON with CLASP binary encoding
...
```
Hex: 53 01 00 11 21 07 00 05 2F 74 65 73 74 40 09 1E B8 51 EB 85 1F

Breakdown:
53          Magic ('S')
01          Flags (version=1, QoS fire, no timestamp)
00 11       Payload length (17 bytes)
[Binary payload]:
  21        Message type: SET (0x21)
  07        Flags: value type F64 (0x07), no revision
  00 05     Address length: 5 bytes
  2F 74 65 73 74   Address: "/test"
  40 09 1E B8 51 EB 85 1F   Value: 3.14 (f64 big-endian)
```

## A.2 HELLO Message

```
Hex: 53 01 00 0F 01 03 F8 00 07 4D 79 20 41 70 70 00 00

Breakdown:
53          Magic ('S')
01          Flags (version=1)
00 0F       Payload length (15 bytes)
[Binary payload]:
  01        Message type: HELLO (0x01)
  03        Protocol version: 3
  F8        Features: param|event|stream|gesture|timeline
  00 07     Name length: 7 bytes
  4D 79 20 41 70 70   Name: "My App"
  00 00     Token length: 0 (no token)
```
...
|---------|------|---------|
| 1.0 | 2026-01 | Initial release: Binary encoding, full protocol spec |
