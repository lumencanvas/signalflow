...
async fn benchmark_120hz_touch() {
    ...
    sender
        .gesture("/touch/1", 1, GesturePhase::Start, Value::Float(0.0))
        .await
        .unwrap();
    ...
    for i in 0..120 {
        ...
        sender
            .gesture("/touch/1", 1, GesturePhase::Move, Value::Map(payload))
            .await
            .unwrap();
    }

    sender
        .gesture("/touch/1", 1, GesturePhase::End, Value::Float(1.0))
        .await
        .unwrap();
    ...
    let reduction =
        ((received_without as f64 - received_with as f64) / received_without as f64) * 100.0;
    ...
    if received_with < received_without {
        println!(
            "  ✅ Coalescing reduces messages by {} ({:.1}%)",
            messages_saved, reduction
        );
    } else {
...
async fn benchmark_240hz_pen() {
    ...
    let start = Instant::now();
    sender
        .gesture("/pen/1", 1, GesturePhase::Start, Value::Float(0.0))
        .await
        .unwrap();
    ...
    sender
        .gesture("/pen/1", 1, GesturePhase::End, Value::Float(1.0))
        .await
        .unwrap();
    ...
    if received < messages_sent {
        println!(
            "  ✅ Coalescing working - {} messages reduced",
            messages_sent - received
        );
    }
    println!();
...
async fn benchmark_fanout() {
    ...
    let start = Instant::now();
    sender
        .gesture("/fanout/touch", 1, GesturePhase::Start, Value::Float(0.0))
        .await
        .unwrap();
    ...
    sender
        .gesture("/fanout/touch", 1, GesturePhase::End, Value::Float(1.0))
        .await
        .unwrap();
    ...
    let reduction =
        ((expected_without as f64 - total_received as f64) / expected_without as f64) * 100.0;

    println!("  Messages sent:        {} (Start + 120 moves + End)", 122);
    println!("  Subscribers:          10");
    println!(
        "  Total received:       {} messages (all subscribers)",
        total_received
    );
...
async fn benchmark_multitouch() {
    ...
    println!(
        "  Messages sent:         {} (10 starts + 600 moves + 10 ends)",
        expected_without
    );
    println!("  Messages received:    {}", received);
    println!(
