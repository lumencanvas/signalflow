...
async fn test_registration_throughput() {
    ...
    assert!(
        throughput > 100.0,
        "Should handle at least 100 registrations/second"
    );
...
async fn test_discovery_latency() {
    ...
    assert!(
        histogram.value_at_quantile(0.95) < 10_000,
        "P95 should be < 10ms"
    );
    assert!(
        histogram.value_at_quantile(0.99) < 50_000,
        "P99 should be < 50ms"
    );
...
async fn test_concurrent_discovery_load() {
    ...
    let start = Instant::now();
    let mut handles: Vec<
        tokio::task::JoinHandle<std::result::Result<Vec<RegisteredDevice>, reqwest::Error>>,
    > = vec![];
    ...
    println!("  Time elapsed: {:?}", elapsed);
    println!(
        "  Throughput: {:.0} discoveries/second",
...
async fn test_ttl_expiration_accuracy() {
    ...
    assert_eq!(
        devices.len(),
        1,
        "Device should still be discoverable after 1s"
    );
...
async fn test_real_world_scale() {
    ...
    let mut handles: Vec<
        tokio::task::JoinHandle<
            std::result::Result<clasp_discovery::rendezvous::RegistrationResponse, reqwest::Error>,
        >,
    > = vec![];
    ...
    println!(
        "  Studio devices (default limit 100): {}",
        studio_devices.len()
    );
    ...
    assert!(
        (all_studio.len() as f64 - 333.0).abs() < 10.0,
        "Tag filtering should work with higher limit"
    );
    assert!(
        discover_time.as_millis() < 1000,
        "Discovery should be fast even with 1000 devices"
    );
