...
async fn benchmark_wildcard_cost(pattern_type: &str) -> BenchmarkResult {
    ...
    for i in 0..msg_count {
        ...
        let _ = sender
            .set(
                &format!("/lights/zone{}/fixture{}/brightness", zone, fixture),
                i as f64,
            )
            .await;
    }
...
async fn benchmark_state_overhead(signal_type: &str) -> BenchmarkResult {
    ...
    for i in 0..msg_count {
        match signal_type {
            ...
            "event" => {
                let _ = publisher.emit("/bench/event", i as f64).await;
            }
            "stream" => {
                let _ = publisher.stream("/bench/stream", i as f64).await;
            }
            _ => {
                let _ = publisher.set("/bench/value", i as f64).await;
            }
...
async fn benchmark_late_joiner(param_count: usize) -> BenchmarkResult {
    ...
    BenchmarkResult {
        ...
        throughput: delivered as f64 / elapsed.as_secs_f64(),
        latency_avg_us: if delivered > 0 {
            elapsed.as_micros() as f64 / delivered as f64
        } else {
            0.0
        },
