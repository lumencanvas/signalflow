...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    subscriber
        .subscribe("/lights/zone50/fixture5/brightness", move |_, addr| {
    ...
    subscriber
        .subscribe("/lights/zone50/*/brightness", move |_, addr| {
    ...
    publisher
        .set("/lights/zone50/fixture5/brightness", 1.0)
        .await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    println!("\n--- Sending /lights/zone50/fixture0/brightness ---");
    publisher
        .set("/lights/zone50/fixture0/brightness", 2.0)
        .await?;
    tokio::time::sleep(Duration::from_millis(100)).await;

    println!("\n--- Sending /lights/zone0/fixture0/brightness ---");
    publisher
        .set("/lights/zone0/fixture0/brightness", 3.0)
        .await?;
    tokio::time::sleep(Duration::from_millis(200)).await;

    println!("\n=== RESULTS ===");
    println!(
        "Exact count:  {} (expected: 1)",
        exact_count.load(Ordering::Relaxed)
    );
    println!(
        "Single count: {} (expected: 2)",
        single_count.load(Ordering::Relaxed)
    );
    println!(
        "Glob count:   {} (expected: 3)",
        glob_count.load(Ordering::Relaxed)
    );
