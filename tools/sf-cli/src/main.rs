...
//! with SignalFlow servers and devices.
...
#[derive(Parser)]
#[command(name = "sf")]
#[command(about = "SignalFlow CLI - Debug and test SignalFlow connections")]
#[command(version)]
struct Cli {
...
#[derive(Subcommand)]
enum Commands {
    /// Discover SignalFlow devices on the network
    Discover {
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Discover { timeout } => {
            println!("Discovering SignalFlow devices...");
        ...
        Commands::Get { address } => {
            let client = SignalFlow::builder(&cli.url)
                .name("sf-cli")
                .connect()
                .await?;
