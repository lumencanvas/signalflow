//! SignalFlow Test Utility
//!
//! Integration tests and stress testing for SignalFlow.
...
#[command(about = "SignalFlow Test Utility")]
#[command(version)]
struct Cli {
    /// Server URL
...
async fn main() -> Result<()> {
    ...
    println!("SignalFlow Test Utility\n");
...
async fn test_connect(url: &str) -> Result<()> {
    ...
    let client = SignalFlow::builder(url)
        .with_name("sf-test-connect")
        .connect()
        .await?;
...
async fn test_multi_client(url: &str, num_clients: usize, messages: usize) -> Result<()> {
    ...
    for i in 0..num_clients {
        ...
        let handle = tokio::spawn(async move {
            let client = SignalFlow::builder(&url)
                .with_name(&format!("sf-test-multi-{}", i))
                .connect()
                .await?;
