...
> Decoders support both binary encoding (default) and MessagePack (legacy compatibility).
...
│             [4]   Encrypted                                     │
│             [3]   Compressed                                    │
│             [2:0] Encoding (000=msgpack/legacy, 001=binary/default) │
│ Byte 2-3:   Payload Length (uint16 big-endian, max 65535)       │
├─────────────────────────────────────────────────────────────────┤
│ [If timestamp flag] Bytes 4-11: Timestamp (uint64 µs)           │
├─────────────────────────────────────────────────────────────────┤
│ Payload (binary encoding or MessagePack for legacy compatibility) │
└─────────────────────────────────────────────────────────────────┘
```
...
**Compact Binary (Default)** — Encoding bits `001`:

CLASP uses positional binary encoding for maximum efficiency:
...
**MessagePack (Legacy)** — Encoding bits `000`:

Still supported for backward compatibility. Decoders auto-detect based on
first payload byte: MessagePack map prefix (0x80-0x8F, 0xDE, 0xDF) indicates MessagePack encoding.

### 2.2.2 Why Binary Encoding?

CLASP uses compact binary encoding instead of MessagePack because:
...
- Subprotocol: `clasp` (binary encoding is default, MessagePack accepted for compatibility)
- Binary mode only (no text frames)
- Ping/Pong: Use WebSocket native ping/pong for keepalive

```javascript
const ws = new WebSocket('wss://localhost:7330/clasp', 'clasp');
...
LumenCanvas._clasp._tcp.local. 
  TXT "version=1" "name=LumenCanvas Studio" "features=psetg" "ws=7330"
...
## 6.1 Scalar Types (Binary Encoding)
...
|---------|------|---------|
| 1.0 | 2026-01 | Initial specification: Compact binary encoding, discovery, P2P, bridges, timing |
